...
async function checkSite(site: Site, apiUrl: string, workerSecret: string) {
  ...
  try {
    ...
    if (status >= 500) {
      await reportAnomaly(apiUrl, workerSecret, {
        site_id: site.id,
        type: "wsod",
        severity: "high",
...
async function reportAnomaly(apiUrl: string, workerSecret: string, payload: any) {
  try {
...
async function fetchSites(apiUrl: string, workerSecret: string): Promise<Site[]> {
  const res = await fetch(`${apiUrl}/api/probes/sites`, {
...
export default {
  async scheduled(event: any, env: Env, ctx: any): Promise<void> {
    ...
    try {
      const sites = await fetchSites(env.SYNTAXWP_API_URL, env.CF_WORKER_SECRET);
      console.log(`Fetched ${sites.length} sites to probe.`);

      // Check all sites in parallel
      const checks = sites.map((site) => checkSite(site, env.SYNTAXWP_API_URL, env.CF_WORKER_SECRET));
